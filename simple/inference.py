...
while True:
    # simulate some data
    df1 = pd.DataFrame({'X1':np.random.standard_normal(200), 
    ...
    df1.to_csv('simple/stage2.csv', index=False)

    rpy.r.assign('X', df1['X1'])
    mytest = int(rpy.r('abs(mean(X)) > 2 / sqrt(length(X))'))
    print(mytest)
    if mytest:
        break
...
nbpath = 'simple/simple.ipynb'
nb = nbformat.read(nbpath, nbformat.NO_CONVERT)

# Initialize the analysis preprocessor (for full dataset)
analysis_pp = preprocessors.AnalysisPreprocessor(timeout=600, nb_log_name='simple/analysis_log.ipynb')
...
simulate_pp = preprocessors.SimulatePreprocessor(timeout=600,
                                                 ...
                                                 nb_log_name='simple/simulate_log.ipynb')
...
nbformat.write(simulate_pp.nb_log, open('simple/simulate_log_final.ipynb', 'w'))
...
pickle.dump(resources, open('simple/simple_info.pckl', 'wb'))
...
if os.path.exists('simple/results_simple.csv'):
    df = pd.read_csv('simple/results_simple.csv')
    df = pd.concat([df, new_df])
else:
    df = new_df
df.to_csv('simple/results_simple.csv', index=False)
