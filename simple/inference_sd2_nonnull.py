...
while True:
    # simulate some data
    delta = np.random.standard_normal() / np.sqrt(280)
...
if os.path.exists('simple/results_simple_sd2_nonnull.csv'):
    df = pd.read_csv('simple/results_simple_sd2_nonnull.csv')
    df = pd.concat([df, new_df])
else:
    df = new_df
df.to_csv('simple/results_simple_sd2_nonnull.csv', index=False)
