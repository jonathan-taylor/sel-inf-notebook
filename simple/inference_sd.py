...
nbpath = 'simple/simple-sd.ipynb'
nb = nbformat.read(nbpath, nbformat.NO_CONVERT)

# Initialize the analysis preprocessor (for full dataset)
analysis_pp = preprocessors.AnalysisPreprocessor(timeout=600, nb_log_name='simple/analysis_log_sd.ipynb')
...
simulate_pp = preprocessors.SimulatePreprocessor(timeout=600,
                                                 ...
                                                 nb_log_name='simple/simulate_log_sd.ipynb')
...
nbformat.write(simulate_pp.nb_log, open('simple/simulate_log_final_sd.ipynb', 'w'))
...
pickle.dump(resources, open('simple/simple_info_sd.pckl', 'wb'))
...
if os.path.exists('simple/results_simple_sd.csv'):
    df = pd.read_csv('simple/results_simple_sd.csv')
    df = pd.concat([df, new_df])
else:
    df = new_df
df.to_csv('simple/results_simple_sd.csv', index=False)
